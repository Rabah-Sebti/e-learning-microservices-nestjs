...
import { CoursesParamsDTO, Status, UpdateProgressDTO } from './dto';
...
export class CourseService {
  ...
  async getUserCourses(userId: number) {
      ...
      select: {
        courses: {
          select: {
            ...
            category: { select: { title: true } },
            sub_category: { select: { title: true } },
            lessons: { select: { progress: { select: { watched: true } } } },
            // course_sub_category: true,
            // progress: true,
    ...
    const progressCourses = await this.prisma.lessonProgress.findMany();

    // con
    // st cour=await this.prisma.course.findMany({
    //   where:{
    //     course_order:{
    ...
    const formattedCourses = userCourses.map((course) => {
      const lessonsWatched = course.lessons.filter((les) =>
        les.progress.some((progress) => progress.watched),
      ).length;
      const totalLessons = course._count.lessons;
      const progressPercentage =
        totalLessons > 0 ? (lessonsWatched / totalLessons) * 100 : 0;
      const totalReviews = course.reviews.length;
      const overallRate =
        totalReviews > 0
          ? course.reviews.reduce((acc, review) => acc + review.rate, 0) /
